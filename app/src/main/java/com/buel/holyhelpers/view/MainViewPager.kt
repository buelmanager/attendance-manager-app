...
import com.buel.holyhelpers.data.CommonData.selectCorpInfo
...
object MainViewPager {
    ...
    fun setDataAndTime(dialogSelectListner: OnDialogSelectListner) {
        if (!selectCorpInfo || context == null) {
            return
        }
        datePickerDialog(
                context!!,
    ...
    fun setViewPager(viewPager: ViewPager?) {
        MainViewPager.viewPager = viewPager!!
    }
    ...
    fun setPagerAdapter(pagerAdapter: FragmentPagerAdapter?) {
        MainViewPager.pagerAdapter = pagerAdapter!!
    }
}